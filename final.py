...
#		solve_button.setStyleSheet("QPushButton {background-color: yellow; color: black;border :1px solid black;}")
		...
		# creating push button to restart the score
		camera__results_button = QPushButton("Camera Results", self)
		# setting geometry
		camera__results_button.setGeometry(250, 600, 200, 50)
		# adding action action to the reset push button
		camera__results_button.clicked.connect(self.import_camera_results)

	def import_camera_results(self):
		print("Importing results")
		results_camera = imageProcessing.start(method='rgb')
		contador = 0
		for cnt, k in enumerate(self.faces.keys()):
			for i in range(3):
				for j in range(3):
					...
					if (j==1) and (i==1):
						...
						self.faces[k][j][i].setStyleSheet(self.bgColors[results_camera[contador]])
					contador += 1
...
class MainCameraWindow(QMainWindow):
	...
	def next_image_file_name(self):
		...
		pattern = f"{pictures_location}/{{:03d}}.jpg"
